...
def make_forecast(model, metadata, periods):
    ...
    # EXTRACT HOLIDAYS - Prophet with add_country_holidays doesn't create a 'holidays' column automatically
    if metadata.get('holidays', False):
        # Try method 1: use train_holiday_names
        try:
            if hasattr(model, 'train_holiday_names') and model.train_holiday_names is not None and len(model.train_holiday_names) > 0:
                holiday_cols = [col for col in model.train_holiday_names if col in forecast.columns]
                if holiday_cols:
                    forecast['holidays'] = forecast[holiday_cols].sum(axis=1)
                else:
                    # No holiday columns found, use fallback method
                    raise ValueError("No holiday columns found")
            else:
                # No train_holiday_names, use fallback method
                raise ValueError("No train_holiday_names")
        except:
            # Fallback: extract from additive_terms by subtracting other components
            if 'additive_terms' in forecast.columns:
                forecast['holidays'] = forecast['additive_terms'].copy()
            else:
                forecast['holidays'] = pd.Series(0, index=forecast.index)
            
            # Subtract all known seasonalities
            if metadata.get('weekly', False) and 'weekly' in forecast.columns:
                forecast['holidays'] = forecast['holidays'] - forecast['weekly']
            if metadata.get('yearly', False) and 'yearly' in forecast.columns:
                forecast['holidays'] = forecast['holidays'] - forecast['yearly']
            if metadata.get('monthly', False) and 'monthly' in forecast.columns:
                forecast['holidays'] = forecast['holidays'] - forecast['monthly']
            if metadata.get('weekly_prepost', False):
                if 'weekly_pre_covid' in forecast.columns:
                    forecast['holidays'] = forecast['holidays'] - forecast['weekly_pre_covid']
                if 'weekly_post_covid' in forecast.columns:
                    forecast['holidays'] = forecast['holidays'] - forecast['weekly_post_covid']
            
            # Subtract regressors
            if metadata.get('regressors') is not None:
                for reg in metadata['regressors']:
                    if reg in forecast.columns:
                        forecast['holidays'] = forecast['holidays'] - forecast[reg]
