...
# Function to make forecast - CORRECTED FOR add_country_holidays
def make_forecast(model, metadata, periods):
    ...
    # FIX: Extract holidays component properly for models using add_country_holidays
    if metadata.get('holidays', False):
        # Method 1: Try to use train_holiday_names (most reliable)
        if hasattr(model, 'train_holiday_names') and model.train_holiday_names:
            # Sum all individual holiday columns
            ...
            # Method 2: Calculate from additive_terms by subtracting known seasonalities
            # additive_terms = holidays + seasonalities + regressors
            forecast['holidays'] = forecast.get('additive_terms', pd.Series(0, index=forecast.index)).copy()
            
            # Subtract seasonalities from additive_terms
            ...
            # Subtract extra regressors if present
            if metadata.get('regressors') is not None:
