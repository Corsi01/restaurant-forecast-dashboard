...
def load_model(model_path, metadata_path):
    ...
    try:
        ...
        # Check if model is nested under 'model' key
...
def plot_seasonalities(forecast, metadata, selected_metric):
    """
    Create separate plots for each seasonality component
    """
    ...
    if metadata.get('yearly', False) and 'yearly' in forecast.columns:
        ...
        forecast_filtered['yearly'] = forecast[['ds', 'yearly']].iloc[:min(365, len(forecast))]
    ...
    if metadata.get('weekly', False) and 'weekly' in forecast.columns:
        ...
        forecast_filtered['weekly'] = forecast[['ds', 'weekly']].iloc[:min(7, len(forecast))]
    ...
    if metadata.get('monthly', False) and 'monthly' in forecast.columns:
        ...
        forecast_filtered['monthly'] = forecast[['ds', 'monthly']].iloc[:min(30, len(forecast))]
    ...
    if metadata.get('weekly_prepost', False) and 'weekly_prepost' in forecast.columns:
        ...
        forecast_filtered['weekly_prepost'] = forecast[['ds', 'weekly_prepost']].iloc[:min(7, len(forecast))]
    ...
    for idx, (seasonality, name) in enumerate(zip(seasonalities, seasonality_names), start=1):
        seasonality_data = forecast_filtered[seasonality]
        
        fig.add_trace(
            go.Scatter(
                x=seasonality_data['ds'],
                y=seasonality_data[seasonality],
                mode='lines',
                name=name,
