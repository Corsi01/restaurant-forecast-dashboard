...
def plot_trend_holidays(model, metadata, selected_metric):
    """
    Create a single plot showing Trend + Holidays + External Regressors
    Uses historical data with proper regressor values to show correct holiday effects
    """
    # Get historical data
    historical_df = model.history.copy()
    
    # CRITICAL: Add regressors from the ORIGINAL training data
    # Prophet stores the original training dataframe internally
    if metadata.get('regressors') is not None:
        regressors_list = metadata['regressors']
        
        # Try to get original regressor values from model's training data
        # Prophet stores extra columns used during fit
        for regressor_name in regressors_list:
            # Check if the regressor data is available in the model
            if hasattr(model, 'train_component_cols') and regressor_name in model.train_component_cols.get('additive_regressors', []):
                # The regressor was used during training - but we don't have its values
                # We need to set dummy values or retrieve from somewhere
                # For now, set to 0 (won't affect holidays, only extra_regressors)
                historical_df[regressor_name] = 0
            else:
                historical_df[regressor_name] = 0
    
    # Predict on historical data WITH regressors (even if zeros)
    historical_forecast = model.predict(historical_df)
    
    # DEBUG: Show what's in the forecast
    st.write("**Debug Info:**")
    st.write(f"Forecast columns: {list(historical_forecast.columns)}")
    if 'holidays' in historical_forecast.columns:
        st.write(f"Holidays sum: {historical_forecast['holidays'].sum():.2f}")
    if 'extra_regressors' in historical_forecast.columns:
        st.write(f"Extra regressors sum: {historical_forecast['extra_regressors'].sum():.2f}")
    ...
    # Add holidays (Italian national holidays from add_country_holidays)
    if 'holidays' in historical_forecast.columns:
        fig.add_trace(
            go.Scatter(
                x=historical_forecast['ds'],
                y=historical_forecast['holidays'],
                mode='lines',
                name=f'Holidays (sum={historical_forecast["holidays"].sum():.2f})',
                line=dict(color='red', width=2, dash='dot')
            )
        )
    
    # Add extra regressors (COVID effects)
    if 'extra_regressors' in historical_forecast.columns:
        fig.add_trace(
            go.Scatter(
                x=historical_forecast['ds'],
                y=historical_forecast['extra_regressors'],
                mode='lines',
                name=f'External Regressors (sum={historical_forecast["extra_regressors"].sum():.2f})',
...
def preprocess_csv(df, selected_metric):
    ...
    - Rename metric column (scontrini/totale) to 'y'
    ...
    # Map English metric names back to Italian column names
    metric_mapping = {
        'Number of Bills': 'scontrini',
        'Total Sales': 'totale'
    }
    metric_lower = metric_mapping.get(selected_metric, selected_metric.lower())
    ...
    if 'ds' not in df_processed.columns or 'y' not in df_processed.columns:
        raise ValueError(f"Could not find required columns. Looking for date column and '{metric_lower}' column")
...
def plot_forecast(model, forecast, actual_data=None, title="Forecast", show_last_months=2):
    ...
    Parameters:
    - show_last_months: int or None. If None, shows all history.
    """
    fig = go.Figure()
    ...
    if show_last_months is None:
        # Show all history
        recent_history = model.history
    else:
        # Show last N months
        cutoff_date = last_historical_date - pd.DateOffset(months=show_last_months)
        recent_history = model.history[model.history['ds'] >= cutoff_date]
...
with tab1:
    ...
    if model is not None and metadata is not None:
        ...
        if 'history_period_tab1' not in st.session_state:
            st.session_state.history_period_tab1 = "2 months"
        
        history_period = st.radio(
            "Historical data to show:",
            options=["2 months", "6 months", "1 year", "All"],
            horizontal=True,
            key="history_period_tab1"
        )
        ...
        months_to_show = history_mapping[history_period]
        ...
        # Display plot if forecast exists
        if 'current_forecast' in st.session_state:
            ...
            fig = plot_forecast(model, forecast, 
                               ...
                               show_last_months=months_to_show)
...
with tab2:
    ...
    actual_column_name = column_name_map.get(selected_metric, selected_metric.lower())
    
    st.info(f"Upload a CSV file with 'data' column (date) and '{actual_column_name}' column (value)")
    ...
    if uploaded_file is not None:
        ...
        try:
            ...
            # History period selector (before Retrain button)
            if 'history_period_tab2' not in st.session_state:
                st.session_state.history_period_tab2 = "2 months"
            
            history_period_tab2 = st.radio(
                "Historical data to show:",
                options=["2 months", "6 months", "1 year", "All"],
                horizontal=True,
                key="history_period_tab2"
            )
            ...
            months_to_show = history_mapping[history_period_tab2]
            ...
            if st.button("Retrain Model & Forecast", key="retrain_btn"):
                with st.spinner("Retraining model... This may take a moment."):
                    try:
                        ...
                        # Store in session state
                        st.session_state.retrained_model = new_model
                        st.session_state.retrained_forecast = new_forecast
            ...
            # Display retrained forecast if exists
            if 'retrained_forecast' in st.session_state and 'retrained_model' in st.session_state:
                new_forecast = st.session_state.retrained_forecast
                new_model = st.session_state.retrained_model
    ...
    if model is not None and metadata is not None:
        ...
        if uploaded_actual is not None:
            ...
            try:
                ...
                st.error(f"Error preprocessing data: {str(e)}")
                # Map display name to actual column name
                actual_column = 'scontrini' if selected_metric == 'Number of Bills' else 'totale'
                st.info(f"Make sure your CSV has 'data' column and '{actual_column}' column")
    ...
    if model is not None and metadata is not None:
        ...
        if st.button("Show Components", key="components_btn"):
            with st.spinner("Generating component analysis..."):
                # For seasonalities, we still need a forecast
                ...
                if st.session_state.component_view == "trend":
                    # Pass model directly for historical analysis
                    fig = plot_trend_holidays(model, metadata, selected_metric)
                else:
