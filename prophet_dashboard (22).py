...
def plot_seasonalities(forecast, metadata, selected_metric):
    """
    Create separate plots for each seasonality component with generic labels
    """
    from plotly.subplots import make_subplots
    
    # Collect seasonalities
    seasonalities = []
    seasonality_names = []
    ...
    if metadata.get('yearly', False) and 'yearly' in forecast.columns:
        ...
        data = forecast[['yearly']].iloc[:min(365, len(forecast))].copy()
        data['x_label'] = [f"Day {i+1}" for i in range(len(data))]
    ...
    if metadata.get('weekly', False) and 'weekly' in forecast.columns:
        ...
        data = forecast[['weekly']].iloc[:min(7, len(forecast))].copy()
        data['x_label'] = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'][:len(data)]
        seasonality_data_list.append(data)
    
    if metadata.get('monthly', False) and 'monthly' in forecast.columns:
        seasonalities.append('monthly')
        seasonality_names.append('Monthly Seasonality')
        data = forecast[['monthly']].iloc[:min(30, len(forecast))].copy()
        data['x_label'] = [f"Day {i+1}" for i in range(len(data))]
        seasonality_data_list.append(data)
        # Add note about monthly
        seasonality_names[-1] = 'Monthly Seasonality (Average Pattern)'
    
    if metadata.get('weekly_prepost', False) and 'weekly_prepost' in forecast.columns:
        seasonalities.append('weekly_prepost')
        seasonality_names.append('Weekly Pre/Post COVID')
        data = forecast[['weekly_prepost']].iloc[:min(7, len(forecast))].copy()
        data['x_label'] = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'][:len(data)]
        seasonality_data_list.append(data)
    ...
    if len(seasonalities) == 0:
        ...
        fig = go.Figure()
        fig.add_annotation(
            text="No seasonalities enabled for this model",
    ...
    for idx, (seasonality, name, data) in enumerate(zip(seasonalities, seasonality_names, seasonality_data_list), start=1):
        ...
        fig.update_xaxes(title_text="Period", row=idx, col=1)
    ...
    # Add info annotation for monthly if present
    if 'monthly' in seasonalities:
        fig.add_annotation(
            text="Note: Monthly seasonality shows the average pattern over a typical month",
            xref="paper", yref="paper",
            x=0.5, y=-0.05,
            showarrow=False,
