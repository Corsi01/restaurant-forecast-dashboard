...
def plot_trend_holidays(model, metadata, selected_metric):
    ...
    Uses historical data to show actual holiday effects including COVID periods
    """
    # Predict on historical data to get holidays effect
    ...
    if metadata.get('holidays', False) and 'holidays' in historical_forecast.columns:
        fig.add_trace(
            go.Scatter(
                x=historical_forecast['ds'],
    ...
    fig.update_layout(
        title="Trend & Holidays Effect (Historical Data)",
        xaxis_title='Date',
