...
# Function to make forecast - FIXED VERSION WITH HOLIDAYS EXTRACTION
def make_forecast(model, metadata, periods):
    ...
    # FIX: Extract holidays component properly
    # Prophet stores holiday effects in separate columns, we need to aggregate them
    if metadata.get('holidays', False):
        # Get all holiday-related columns
        holiday_cols = []
        
        # Try to get holiday names from model
        if hasattr(model, 'train_holiday_names'):
            holiday_cols = [col for col in forecast.columns if col in model.train_holiday_names]
        
        # If no train_holiday_names or empty, identify holiday columns by exclusion
        if not holiday_cols:
            # List of known Prophet component columns to exclude
            exclude_cols = {
                ...
                'yhat', 'pre_covid', 'post_covid',
                'weekly_pre_covid', 'weekly_post_covid',
                'weekly_pre_covid_lower', 'weekly_pre_covid_upper',
                'weekly_post_covid_lower', 'weekly_post_covid_upper'
            }
            
            # Also exclude regressor columns
            if metadata.get('regressors') is not None:
                exclude_cols.update(metadata['regressors'])
            
            # Find columns that are not in exclude list and don't have _lower or _upper suffix
            holiday_cols = [col for col in forecast.columns 
                          if col not in exclude_cols and not col.endswith('_lower') and not col.endswith('_upper')]
        
        # Sum all holiday effects into a single 'holidays' column
        if holiday_cols:
            forecast['holidays'] = forecast[holiday_cols].sum(axis=1)
        else:
            # If we still can't find holiday columns, set to zero
            forecast['holidays'] = 0
...
def plot_trend_holidays(forecast, metadata, selected_metric):
    ...
    if metadata.get('holidays', False) and 'holidays' in forecast.columns:
        # Check if holidays column has any non-zero values
        if (forecast['holidays'] != 0).any():
            fig.add_trace(
                go.Scatter(
                    x=forecast['ds'],
                    y=forecast['holidays'],
            ...
            st.warning("Holidays are enabled in metadata but no holiday effects found in forecast")
